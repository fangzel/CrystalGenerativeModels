# adapted from commit
# b700b7c30338d8351b35428a887854ba214d73b8
...
class AseAtomsDataset(Dataset, ABC):
    ...
    def __init__(self, config, attr_data_type = None,transform=None, atoms_transform=apply_one_tags) -> None:
        self.config = config
        self.attr_data_type = attr_data_type
    ...
    def __getitem__(self, idx):
        ...
        if self.attr_data_type == 'pkl':
            setattr(data_object, self.attr_name, torch.tensor(self.pkl_data[self.attr_name][idx], dtype=torch.float))

        elif self.attr_data_type == 'cif_pkl':
...
class AseReadDataset(AseAtomsDataset):
    ...
    def load_dataset_get_ids(self, config) -> List[Path]:
        ...
        if config.get("files", None) is None:
            ...
            return [self.path / f for f in config["files"]]
